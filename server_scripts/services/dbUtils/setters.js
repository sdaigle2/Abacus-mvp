...
function updateOrInsertAllEntries(argsObj, cb) {
	...
	async.map(entries, function (entry, cb) {
		console.log('updateOrInsertAllEntries')
		if (_.isObject(entry)) {
			...
			if (entry[idField] && isValidID(entry[idField])) {
				...
				// dbService.insertDBfunction(db,entryID, function (err, res) {
				dbInsert(db, entry, entryID, function (err, res) {
				...
				// dbService.insertDBfunction(db,entryID, function (err, res) {
				dbInsert(db, entry, function (err, res) {
			...
			console.log('We re being given the ID for an object')
			...
			dbService.findDBfunction(db,entryID, function (err, entryValue) {
				if (err) {
					cb(err);
				} else {
...
function insertDesign(db, designValue, id, cb) {
	console.log('in insert Desgin function')
	if (_.isFunction(id)) {
		console.log("if")
		...
		// 		cb(err);
		// 	} else {
				console.log('inserting into design: ')
				dbService.insertDBfunction('design',designValue, (err, res) => {
...
function updateLinkedOrderFields(orderObj, cb) {
	console.log("in update linked order fields")
	...
	if (_.isFunction(id)) {
		...
		updateLinkedOrderFields(order, function (err, updatedOrder) {
			...
			if (err) {
				...
				console.log("inserting this to the orders")
...
function updateLinkedUserFields(userObj, cb) {
	...
	var updateSavedDesigns = function (cb) {
		console.log("updateSavedDesigns")
	...
	var updateCart = function (cb) {
		...
		if (_.isString(cart)) {
			...
			dbService.findDBfunction('orders',cart, cb);
			...
			console.log("} else if (_.isObject(cart)) {")
	...
	async.parallel({
		...
		if (err) {
			...
			console.log("in parallel:",results.savedDesigns)
